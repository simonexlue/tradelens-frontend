...
export default function LoginPage() {
    ...
    function authError(message: string) {
        ...
        if (/email rate limit/i.test(message)) return "Too many attempts. Please wait a moment and try again.";
        return message;
    }
    ...
    async function handleAuth(e: React.FormEvent) {
        ...
        try {
            if(isSignUp) {
                // Sign Up : New User
                // Send link using Supabase email provider
        ...
        <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <form onSubmit={handleAuth} noValidate className="space-y-5">
                ...
                <input
                    ...
                    setEmail(v);
                    setEmailError(validateEmail(v) ? null : "Please enter a valid email.");
                    }}
                    ...
                    emailError ? "border-rose-500 focus:border-rose-500" : "border-slate-700 focus:border-teal-500"
                    }`}
                />
                {emailError && (
                    <p id="email-error" className="mt-1 text-xs text-rose-400 px-1">
                    {emailError}
                    </p>
                )}
                </label>
                ...
                >
                    {loading ? (isSignUp ? "Creating..." : "Signing in") : isSignUp ? "Create" : "Sign In"}
                </Button>
                </div>
                ...
                >
                    {status.type === "err" && status.msg.includes("Password should contain") ? (
                    <>
                        <strong className="block mb-1 font-medium text-rose-400">Password must include:</strong>
